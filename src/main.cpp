...
unsigned int nRetarget = 40;
unsigned int nStakeMinAge = 12 * 60 * 60; // 12 hours
unsigned int nStakeMaxAge = -1;           //unlimited
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // every 40 blocks
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing;

static const int64_t nTargetTimespan = 40 * 60;
...
const string strMessageMagic = "NetworkCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // NetworkCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // NetworkCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight <= 8000)
      ...
      {
        int64_t nSubsidy = 0.625 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight > 40000)
      {
        int64_t nSubsidy = 0.3125 * COIN;
        return nSubsidy + nFees;
      }
...
// NetworkCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes NetworkCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // NetworkCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // NetworkCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // NetworkCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // NetworkCoin: track money supply and mint amount info
...
// NetworkCoin: total coin age spent in transaction, in the unit of coin-days.
...
// NetworkCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // NetworkCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // NetworkCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // NetworkCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // NetworkCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // NetworkCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // NetworkCoin: check proof-of-stake
    ...
    // NetworkCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // NetworkCoin: getblocks may not obtain the ancestor block rejected
    ...
    // NetworkCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NetworkCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Wed, 15 Jul 2015 17:08:31 GMT";
        CTransaction txNew;
        txNew.nTime = 1436980111;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd2ff7d3bcb3e3a68f2725b0b39bab07a219c9bbb69b8175e836aa651b45584bb"));
        block.print();
        ...
        // NetworkCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // NetworkCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // NetworkCoin: tell downloading node about the latest block if it's
